...
public class JobInfoServiceImpl implements JobInfoService {
    ...
    @Override
    public void stopClient(JobInfo jobInfo) {
    ...
    @Async("commonAsyncTaskExecutor")
    @Override
    public void pauseClient(JobInfo jobInfo) {
        Long jobId = jobInfo.getJobId();
        Set<String> address = getClientAddress(jobInfo);
        if (CollectionUtils.isNotEmpty(address)) {
            address.forEach(ip -> {
    ...
    private Set<String> getClientAddress(JobInfo jobInfo) {
        ...
        for (JobLog log : logList) {
            ...
            if (executorType == 0 && !ValidUtils.checkAddress(address)) {
    ...
    private void sendRequest(String url, Long jobId) {
        String token = null;
        if (RequestContextHolder.getRequestAttributes() != null) {
            ServletRequestAttributes servletRequestAttributes = (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
            token = servletRequestAttributes.getRequest().getHeader("jwt_token");
        }
        String result;
        try {
            result = RpcClient.stopJob(url, token, objectMapper.writeValueAsString(new JobDataDTO().setJobId(jobId)));
        } catch (Exception e) {
            result = "exception";
        }
