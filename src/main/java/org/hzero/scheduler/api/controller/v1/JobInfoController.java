...
public class JobInfoController extends BaseController {
    ...
    @Permission(level = ResourceLevel.ORGANIZATION)
    @PostMapping("/pause")
    public ResponseEntity<Void> pauseJob(@PathVariable Long organizationId, @RequestBody @Encrypt JobInfo jobInfo) {
        JobInfo info = jobInfoRepository.selectOne(new JobInfo().setTenantId(organizationId).setJobId(jobInfo.getJobId()));
        if (info != null) {
            jobInfoService.pauseClient(info);
    ...
    @Permission(level = ResourceLevel.ORGANIZATION)
    @PostMapping("/stop")
    public ResponseEntity<Void> stopJob(@PathVariable Long organizationId, @RequestBody @Encrypt JobInfo jobInfo) {
        JobInfo info = jobInfoRepository.selectOne(new JobInfo().setTenantId(organizationId).setJobId(jobInfo.getJobId()));
        if (info != null) {
            jobInfoService.stopClient(info);
